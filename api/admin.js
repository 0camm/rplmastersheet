...
export default async function handler(req, res) {
  ...
  if (action === 'setSalary') {
    ...
    await kv.hset('players', { [id]: { ...player, salary: Number(salary) } });
  ...
  if (action === 'setTeam') {
    if (!entry) return res.status(404).json({ error: `Player "${name}" not found` });
    const [id, player] = entry;
    await kv.hset('players', { [id]: { ...player, team } });
  ...
  if (action === 'removePlayer') {
    if (!entry) return res.status(404).json({ error: `Player "${name}" not found` });
    const [id] = entry;
    await kv.hdel('players', id);
    return res.json({ ok: true, message: `${name} removed` });
  }

  if (action === 'addPlayer') {
    // Add a player manually (for pre-seeding existing roster)
  ...
  if (action === 'renamePlayer') {
    if (!entry) return res.status(404).json({ error: `Player "${name}" not found` });
    const [id, player] = entry;
    await kv.hset('players', { [id]: { ...player, name: newName } });
